...
!     
     ...
     &     veold,matname,mi,ielprop,prop,imastload,pmastload)
!     
...
!     
      implicit none
!     
      logical ivolumeforce,interfaceload,faceident
!     
      character*8 lakonl
...
!     
     ...
     &     nelemload(2,*),ielprop(*),null,ids,iface,igs,indexs,jface,
     ...
     &     ipompc(*),nodempc(3,*),nmpc,ikmpc(*),ilmpc(*),iscale,nelems,
     &     imastload(2,*)
!     
     ...
     &     vold(0:mi(2),*),prop(*),pmastload(3,*),
     &     xload(2,*),xi,et,ze,const,xsj,ff(60),weight,ttime,time,
     &     tvar(2),coords(3),dtime,coefmpc(*)
!     
      include "gauss.f"
!     
...
!
!     negative om signals Euler (rotational acceleration) force
              if(om(jj1).gt.0.d0) then
...
!     inclusion of the Euler force into the body force
                bf(1)=bf(1)+(p2(2,jj1)*(q(3)-const*p2(3,jj1))
     &               -p2(3,jj1)*(q(2)-const*p2(2,jj1)))*om(jj1)
                bf(2)=bf(2)+(p2(3,jj1)*(q(1)-const*p2(1,jj1))
     &               -p2(1,jj1)*(q(3)-const*p2(3,jj1)))*om(jj1)
                bf(3)=bf(3)+(p2(1,jj1)*(q(2)-const*p2(2,jj1))
     &               -p2(2,jj1)*(q(1)-const*p2(1,jj1)))*om(jj1)
              endif
            enddo
!     
            do i1=1,3
...
!     
              if(sideload(ids)(3:4).eq.'NU') then
                do k=1,3
                  ...
                  enddo
                enddo
                read(sideload(ids)(2:2),'(i1)') jltyp
                jltyp=jltyp+20
                iscale=1
